...
# 1. 설정 및 즐겨찾기 초기화
ST_VERSION = "V5.0 (Mobile Optimized Table & Color Coding)"
DEFAULT_PLACES = {
...
def get_coords_by_name(name):
    ...
    try:
        res = requests.get(url, headers={'User-Agent': 'WeatherApp/5.0'}, timeout=3).json()
...
# --- UI 구성 ---
st.set_page_config(page_title="Gunsan CC Weather V5.0", layout="centered")
...
if data:
    ...
    # 📊 2단계 핵심 기능: 모바일 최적화 한눈에 보는 직관적 색상 표
    ...
    # 로우(Row)별 글자 색상 스타일 정의 함수
    def style_rows_by_condition(row):
        if "위험" in str(row["현황"]):
            return ['color: #D32F2F; font-weight: bold;'] * len(row)  # 위험은 붉은색
        elif "그늘" in str(row["현황"]):
            return ['color: #2E7D32; font-weight: bold;'] * len(row)  # 좋은건 초록색
        else:
            return ['color: #212121; font-weight: normal;'] * len(row) # 보통은 검은색

    # 스타일 적용 후 표 출력
    styled_mobile_df = df_mobile.style.apply(style_rows_by_condition, axis=1)
    st.dataframe(styled_mobile_df, use_container_width=True, height=500)

    # 📉 3단계 세부 분석: 상세 차트 (하단 배치)
    st.divider()
    st.subheader("📉 정밀 예측 트렌드 그래프")
    tab1, tab2 = st.tabs(["☁️ 구름 밀도 및 상층풍 바람", "🌡️ 기온 변화 추이"])
    
    with tab1:
        st.markdown("**구름두께(막대)가 높고 지표선이 위에 있을수록 쾌적함이 오래 유지됩니다.**")
        st.bar_chart(df_day["구름두께"], color="#90A4AE")
        st.line_chart(df_day["그늘지속성"], color="#00E676")

    with tab2:
        st.markdown("**실제 기온과 체감 기온 비교**")
